...
const endRecord = () => {
  ...
  if (recordActions.value.length > 0) {
    ...
    axios
        .post('/controller/testCases/saveRecordActions', para)
        .then((resp) => {
...
const endRecordEle = () => {
  if (recordEleArr.value.length > 0) {
    ...
    // todo: 后端增加接口 解析录制的元素
    axios
        .post('/controller/testCases/saveRecordElements', para)
        .then((resp) => {
    ...
    >
      ...
      >
        ...
        >
          ...
          <!--录制坐标-->
          <el-tab-pane
              :label="$t('androidRemoteTS.code.recordPoint')"
              name="recordAction"
          >
