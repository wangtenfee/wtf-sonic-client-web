...
const agent = {
  hub: {
    ...
    action: '操作する',
    switch: '切り替える',
    status: '番号の USB ポートがデバイスに接続されていません',
    relate: '協会',
    poweron: '電源オン',
    poweroff: '電源オフ',
    allpoweron: 'すべての電源がオン',
    allpoweroff: 'すべての電源がオフ',
  },
  newAgent: 'エージェント追加',
...
const settingIndexTS = {
  ...
  remote: {
    alertMsg:
      'リモート制御の構成に使用される最大占有時間。それを超えると、現在の占有率が自動的に解除されてアイドルに戻ります。（リモート コントロールと REST API を介して費やされた最大時間のみに影響し、テストを実行しているデバイスには影響しません）',
    text: 'リモート制御の最大消費時間：：',
    idle_alertMsg:
      'リモート制御を構成するための最長アイドル時間。リモート制御ユーザーがこの時間を超えて何の操作もしない場合、現在の占有率は自動的に解除されアイドルに戻ります。（遠隔制御のみに影響し、テストを実行するデバイスは影響を受けない）',
    idle_text: '最長アイドル時間の遠隔制御：',
  },
};
const resourceTS = {
  syncSucceed: '同步成功!',
  syncResource: 'リソースの同期',
  syncResourceInfo: '同期リソースの説明',
  syncResourceInfoMessage:
    'アプリケーション内のすべての要求リソース情報を再フル更新します。一般的なバージョン更新には同期が必要です',
  pageData: {
    idText: '资源id',
    message: '描述',
    path: '路径',
